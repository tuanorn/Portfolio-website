...
function PPP() {
    let pppPgStruct: {[key: string]: any} = {
        "Brainstorming": [{
            ...
            "ImgSrc": "../Images/Electric_propulsion.webp",
            ...
            "ImgSrc": "../Images/Leaf_in_bulb.jpg",
    ...
    let brainstormingItems: JSX.Element[] = pppPgStruct["Brainstorming"].map((item: any) => {
        return <Card className="h-fit mb-3">
            <h5>{item["Title"]}</h5>
            <img className="max-w-[50%] mx-auto" src={require(item["ImgSrc"])}></img>
            <p>{item["Text"]}</p>
        </Card>
    })
